    ...
    public partial class OrderTab : UserControl
    {
            ...
            for (int i = 0; i < listcustomer.Count; i++)
            {
                if (listcustomer[i].Orders.Count > 0)
                {
                    ...
                    if (listcustomer[i].Orders.Count > 1)
                    {
                        ...
                        {
                            dataGridView1.Rows.Add(listcustomer[i].GetId(), listcustomer[i].Orders[j].DateOfCreateOrder, listcustomer[i].Orders[j].Price, $"{address.SetIndex}, {address.SetCountry}, {address.SetCity}, {address.SetStreet}, {address.SetBuilding}, {address.SetApartament}", listcustomer[i].Orders[j].GetStatus, listcustomer[i].SetFullName);
                            DeliveryTimeBox.Text = listcustomer[i].Time;
                        }
                    }
                    else
                    {
                        dataGridView1.Rows.Add(listcustomer[i].GetId(), listcustomer[i].Orders[0].DateOfCreateOrder, listcustomer[i].Orders[0].Price, $"{address.SetIndex}, {address.SetCountry}, {address.SetCity}, {address.SetStreet}, {address.SetBuilding}, {address.SetApartament}", listcustomer[i].Orders[0].GetStatus, listcustomer[i].SetFullName);
                        DeliveryTimeBox.Text = listcustomer[i].Time;
                    }
                }
            }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                {
                    DeliveryTimeBox.Text = time.Time;
                }
